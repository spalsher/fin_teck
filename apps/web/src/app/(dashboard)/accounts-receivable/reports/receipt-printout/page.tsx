...
import { Receipt, Calendar, Printer, FileDown, X, ArrowLeft, Filter, Search, Hash } from 'lucide-react';

type FormatType = 'PDF' | 'Excel' | 'CSV';
interface ReceiptRow { receiptNo: string; date: string; customer: string; invoiceRef: string; method: 'Cash' | 'Bank Transfer' | 'Cheque'; amount: number; status: 'CLEARED' | 'PENDING'; }
...
const MOCK: ReceiptRow[] = [
  { receiptNo: 'RCP-2026-001', date: '2026-02-08', customer: 'Al-Habib Corporation', invoiceRef: 'INV-2026-001', method: 'Bank Transfer', amount: 50000, status: 'CLEARED' },
  { receiptNo: 'RCP-2026-002', date: '2026-02-12', customer: 'Sunrise Trading Co.', invoiceRef: 'INV-2026-002', method: 'Cheque', amount: 100000, status: 'PENDING' },
  { receiptNo: 'RCP-2026-003', date: '2026-02-14', customer: 'Al-Habib Corporation', invoiceRef: 'INV-2026-001', method: 'Cash', amount: 30000, status: 'CLEARED' },
  { receiptNo: 'RCP-2026-004', date: '2026-02-18', customer: 'Metro Enterprises', invoiceRef: 'INV-2026-003', method: 'Bank Transfer', amount: 63250, status: 'CLEARED' },
];

const methodConfig: Record<ReceiptRow['method'], { bg: string; text: string }> = {
...
export default function ARReceiptPrintoutPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Receipt Printout</h1>
          <p className="text-xs text-slate-400 mt-0.5">Accounts Receivable › Reports › Receipt Printout</p>
        </div>
      </div>

      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-emerald-600/30 via-green-600/20 to-slate-800/50 border-b border-white/10 px-6 py-4">
          <div className="flex items-center gap-2"><Filter className="h-4 w-4 text-emerald-400" /><span className="text-sm font-semibold text-white">Report Parameters</span></div>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 items-end">
            <div className="space-y-1.5">
              ...
              <input type="text" value={fromReceipt} onChange={(e) => setFromReceipt(e.target.value)} placeholder="RCP-2026-0001"
                className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:border-emerald-500/60 focus:outline-none font-mono" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)}
                className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white focus:border-emerald-500/60 focus:outline-none [color-scheme:dark]" />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            {(['PDF', 'Excel', 'CSV'] as FormatType[]).map((f) => (
              <button key={f} onClick={() => setFormat(f)}
                className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-all border ${format === f ? 'bg-emerald-600/30 border-emerald-500/50 text-emerald-300' : 'border-white/10 bg-white/5 text-slate-400 hover:text-white'}`}>
                {f}
              </button>
            ))}
          </div>
          <div className="flex flex-wrap gap-2 mt-5 pt-5 border-t border-white/8">
            ...
            <button onClick={handlePrint} disabled={loading}
              className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-2.5 text-sm font-medium text-white hover:from-emerald-500 hover:to-green-500 disabled:opacity-60 transition-all shadow-lg shadow-emerald-500/25">
      ...
      {searched && results.length > 0 && (
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-slate-800/80 to-slate-700/50 border-b border-white/10 px-6 py-3 flex items-center justify-between">
            <span className="text-sm font-semibold text-white">Receipt List</span>
            <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-medium text-emerald-300 ring-1 ring-emerald-500/30">{results.length} receipts</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  {['Receipt No', 'Date', 'Customer', 'Invoice Ref', 'Method', 'Amount', 'Status'].map((h) => (
              ...
              <tbody className="divide-y divide-white/5">
                {results.map((row, idx) => {
                  const mc = methodConfig[row.method];
                  return (
                    <tr key={idx} className="hover:bg-white/5 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-emerald-300">{row.receiptNo}</td>
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-teal-300">{row.invoiceRef}</td>
                      <td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${mc.bg} ${mc.text}`}>{row.method}</span></td>
                      ...
                      <td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${row.status === 'CLEARED' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-amber-500/15 text-amber-400'}`}>{row.status}</span></td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="bg-gradient-to-r from-slate-800 to-slate-700/70 border-t border-white/10">
                  <td colSpan={5} className="px-4 py-3 text-right text-xs font-bold text-slate-300 uppercase">Grand Total</td>
