...
const BANKS = [
  ...
  { id: 'B004', name: 'Meezan Bank - AR Collection' },
];

const MOCK_PENDING: PendingInvoice[] = [
  { id: '1', invoiceNo: 'AR-2026-001', invoiceDate: '2026-02-01', bookingNo: '0163173', originalAmount: 100000, pendingAmount: 100000, toAdjust: '' },
  { id: '2', invoiceNo: 'AR-2026-002', invoiceDate: '2026-02-05', bookingNo: '0163174', originalAmount: 75000, pendingAmount: 75000, toAdjust: '' },
  { id: '3', invoiceNo: 'AR-2026-003', invoiceDate: '2026-02-10', bookingNo: '0163175', originalAmount: 120000, pendingAmount: 80000, toAdjust: '' },
];
...
export default function PaymentReceiptPage() {
  ...
  const netReceivable = (parseFloat(String(form.paymentAmount)) || 0) + totalTax;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Payment Receipt</h1>
          <div className="flex items-center gap-1 text-xs text-slate-400 mt-0.5">
            <span>Accounts Receivable</span>
            <ChevronRight className="h-3 w-3" />
            <span>Transactions</span>
            <ChevronRight className="h-3 w-3" />
            <span className="text-emerald-400">Payment Receipt</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-2xl overflow-hidden mb-4">
        <div className="bg-gradient-to-r from-emerald-600/30 via-teal-600/20 to-slate-800/50 border-b border-white/10 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-sm font-semibold text-white">New Customer Receipt</span>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div className="space-y-1.5">
              <label className="flex items-center gap-1.5 text-xs font-medium text-slate-400 uppercase tracking-wider"><Hash className="h-3 w-3" /> P.R. No</label>
              <input type="text" value={form.prNo} onChange={(e) => setForm({ ...form, prNo: e.target.value })} placeholder="Payment No"
            ...
            <div className="space-y-1.5">
              ...
              <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
                className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white focus:border-emerald-500/60 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 transition-all [color-scheme:dark]" />
            </div>
            ...
            <div className="col-span-2 space-y-1.5">
              <label className="flex items-center gap-1.5 text-xs font-medium text-slate-400 uppercase tracking-wider"><User className="h-3 w-3" /> Company</label>
              <div className="flex gap-2">
                <select value={form.customerId} onChange={(e) => handleCustomerChange(e.target.value)}
                  className="w-28 flex-shrink-0 rounded-lg border border-white/10 bg-slate-800 px-3 py-2.5 text-sm text-white focus:border-emerald-500/60 focus:outline-none transition-all">
                  <option value="">Co ID</option>
                  {CUSTOMERS.map((c) => <option key={c.id} value={c.id}>{c.id}</option>)}
                </select>
                <input type="text" value={form.customerName} readOnly placeholder="Co Name"
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <input type="text" readOnly defaultValue="Posted/Un-Posted"
                  className="w-32 rounded-lg border border-white/10 bg-slate-800/50 px-3 py-2.5 text-xs text-slate-500 focus:outline-none cursor-default" />
              </div>
            </div>
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" className="flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2.5 text-xs text-emerald-400 hover:bg-emerald-500/20 transition-all whitespace-nowrap">
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t border-white/10">
            <button type="button" onClick={handleInsert} disabled={loading || saved}
              className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2 text-sm font-medium text-white hover:from-emerald-500 hover:to-teal-500 disabled:opacity-60 transition-all shadow-lg shadow-emerald-500/20">
            ...
            <button type="button" onClick={() => setShowPending(true)}
              className="flex items-center gap-2 rounded-lg bg-blue-600/20 border border-blue-500/30 px-4 py-2 text-sm font-medium text-blue-400 hover:bg-blue-600/30 transition-all">
              <Search className="h-4 w-4" /> Search
            </button>
            <button type="button" onClick={() => { setForm({ prNo: '', date: today, chequeNo: '', description: '', customerId: '', customerName: '', bankId: '', bankName: '', paymentAmount: '', isPosted: false }); setTaxLines([]); setError(null); setShowPending(false); }}
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-xl overflow-hidden mb-4">
        <div className="bg-gradient-to-r from-slate-800/80 to-slate-700/50 border-b border-white/10 px-6 py-3 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setTaxLines((p) => [...p, emptyTaxLine()])}
            className="flex items-center gap-1.5 rounded-lg bg-emerald-500/20 border border-emerald-500/30 px-3 py-1.5 text-xs font-medium text-emerald-400 hover:bg-emerald-500/30 transition-all">
            <Plus className="h-3.5 w-3.5" /> Add Row Tax
          </button>
        </div>
        <div className="p-4">
          {taxLines.length === 0 ? (
            <div className="py-6 text-center text-xs text-slate-600">No tax entries. Click &ldquo;Add Row Tax&rdquo; to add.</div>
          ) : (
                  ...
                  <tr key={tl.id} className="group hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-2 py-2">
                      <input type="text" value={tl.description} onChange={(e) => setTaxLines((p) => p.map((x) => x.id === tl.id ? { ...x, description: e.target.value } : x))} placeholder="Tax description"
                        className="w-full rounded-md border border-white/10 bg-slate-800/80 px-2 py-1.5 text-xs text-white placeholder-slate-600 focus:border-emerald-500/60 focus:outline-none transition-all" />
                    </td>
          ...
          <div className="flex items-center justify-end gap-6 pt-3 border-t border-white/10">
            ...
            <div className="text-right">
              ...
              <p className="font-mono text-sm font-bold text-emerald-400">{netReceivable.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  {['Invoice No', 'Invoice Date', 'Booking No', 'Invoice Amount (Original)', 'Invoice Amount (Pending)', 'To Be Adjust'].map((h) => (
              ...
              <tbody className="divide-y divide-white/5">
                {pending.map((inv) => (
                  <tr key={inv.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs text-slate-400">{inv.invoiceDate}</td>
                    <td className="px-4 py-2.5 text-xs text-slate-300">{inv.bookingNo}</td>
